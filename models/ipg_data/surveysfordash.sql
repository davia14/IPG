WITH addnames AS (SELECT surv.*, dim.name AS staff_name, dim.cluster FROM {{ref("surveysystemlvlpivoted")}} AS surv LEFT OUTER JOIN ip-ipg-data.dimension_table.pmdim AS dim ON surv.new_system = dim.New_System),
...
AND emails.school_year=surv.school_year_c AND emails.survey_type = surv.survey_type AND emails.question = surv.question)
